...
<?php endif; ?>
		...
		<script src="<?php echo base_url($frameworks_dir . '/ckeditor/ckeditor.js'); ?>"></script>

		<script src="<?php echo base_url($plugins_dir . '/bootstrap-wysihtml5/bootstrap3-wysihtml5.all.min.js'); ?>"></script>


		<div id="debugModal" class="modal fade" role="dialog">
...
$(document).ready(function() {
    $('#debug_btn').click(function() {
		message = "";
		$('.error-debug').each(function() {
			message = message + $(this).html();
		});

		$('#debugModal').find(".modal-body").html(message);
        $('#debugModal').modal();
    });
});
</script>
